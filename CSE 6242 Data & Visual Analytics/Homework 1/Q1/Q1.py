...
class Graph:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        return {node: deg for node, deg in degree_count.items() if deg == max_degree}
...
class  TMDBAPIUtils:
    ...
    def _make_request(self, endpoint: str, params: dict = None) -> dict:
        """
        Helper function to call TMDB API and return parsed JSON
        """
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list[int]=None) -> list:
        ...
        data = self._make_request(f"/3/movie/{movie_id}/credits")
        cast = data.get("cast", [])
        ...
        if limit is not None:
            cast = [c for c in cast if c.get("order", 9999) < limit]
    ...
    def get_movie_credits_for_person(self, person_id: str, start_date: str = None, end_date: str = None) -> list:
        ...
        """
        data = self._make_request(f"/3/person/{person_id}/movie_credits")
...
tmdb_api_utils = TMDBAPIUtils(api_key='0d53679245e889434f2713caad8472dc')

# Build base graph
new_nodes = set()

movie_list = tmdb_api_utils.get_movie_credits_for_person(graph.nodes[0][0], '1999-01-01', '1999-12-31')

for movie in movie_list:
  cast_list = tmdb_api_utils.get_movie_cast(movie['id'], limit=5, exclude_ids=[2975])
  ...
  if len(cast_list) == 0:
    ...
    for cast in cast_list:
      ...
      new_nodes.add(str(cast['id']))

for iter_count in range(0, 2):
  print(iter_count)
  ...
  for node in nodes:
    movie_list = tmdb_api_utils.get_movie_credits_for_person(node, '1999-01-01', '1999-12-31')

    for movie in movie_list:
      cast_list = tmdb_api_utils.get_movie_cast(movie['id'], limit=5, exclude_ids=[node])
      ...
      if len(cast_list) == 0:
        ...
        for cast in cast_list:
          graph.add_node(id=str(cast['id']), name=cast['name'])
          graph.add_edge(source=node, target=str(cast['id']))
